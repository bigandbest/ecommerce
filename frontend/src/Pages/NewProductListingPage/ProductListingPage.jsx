...
        // NOTE: This is inefficient (N+1 queries), but matches the pattern requested. 
...
  // 🎯 MODIFIED: handle ADD/ENQUIRY button click
  const handleProductAction = async (e, item, isEnquiry) => {
    e.preventDefault();
    e.stopPropagation();
...
    const { success, error } = await addToCart(currentUser.id, item.id, 1);
...
                      {isEnquiry ? (
                        <button
                          // 🎯 Call the modified handleProductAction
                          onClick={(e) => handleProductAction(e, item, isEnquiry)} 
