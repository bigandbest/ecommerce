...
import { FaWallet, FaPlus, FaSpinner, FaLock, FaUnlock } from 'react-icons/fa';
...
const WalletSection = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="mb-6">
        <h2 className="text-2xl lg:text-3xl font-bold text-gray-800 mb-2">My Wallet</h2>
      ...
      <div className="bg-gradient-to-br from-purple-500 to-blue-600 rounded-2xl p-6 text-white shadow-lg">
        ...
        <div className="text-center mb-6">
          <div className="text-4xl font-bold mb-2">₹{wallet?.balance?.toFixed(2) || '0.00'}</div>
          {wallet?.is_frozen && (
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-gray-800">₹{wallet.total_recharged?.toFixed(2) || '0.00'}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-gray-800">₹{wallet.total_spent?.toFixed(2) || '0.00'}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <div className="flex items-center gap-3">
              <div className={`p-2 rounded-lg ${wallet.is_frozen ? 'bg-red-100' : 'bg-green-100'}`}>
                {wallet.is_frozen ? <FaLock className="text-red-600" /> : <FaUnlock className="text-green-600" />}
              </div>
              <div>
                <p className="text-sm text-gray-600">Status</p>
                <p className={`text-lg font-bold ${wallet.is_frozen ? 'text-red-600' : 'text-green-600'}`}>
