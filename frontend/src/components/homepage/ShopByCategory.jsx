...
const ShopByCategory = ({ sectionName }) => {
    ...
    try {
      ...
      const transformedCategories = hierarchyData.map((category) => ({
        ...
        subcategories: category.subcategories.map((subcategory) => ({
          ...
          productCount: 0, // We can add this later if needed
        })),
      }));
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="max-w-7xl mx-auto px-4 py-8">
        {/* Page Header */}
        <div className="mb-8 ">
          <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-transparent text-center bg-gradient-to-r from-orange-500 via-orange-500 to-orange-600 bg-clip-text drop-shadow-lg">
        ...
        {categories.map((category) => (
          <div key={category.id} className="mb-12">
            {/* Category Header */}
            <h2 className="font-bold text-gray-900 mb-6">{category.name}</h2>
