...
const AddBannerGroupProducts = () => {
    ...
    const fetchGroupData = async () => {
        try {
            // Fetch the details of the specific Banner Group
            const groupRes = await axios.get(`${API_URL_GROUP}/${id}`);
            setGroup(groupRes.data.bannerGroup);

            // Fetch the products associated with this Banner Group
            const productsRes = await axios.get(`${API_URL_PRODUCTS}/by-group/${id}`);
    ...
    const handleRemoveProduct = async (productId) => {
        const isConfirmed = window.confirm('Are you sure you want to remove this product from the group?');
        if (isConfirmed) {
            try {
...
export default AddBannerGroupProducts;
